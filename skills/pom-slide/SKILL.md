...
| `grow`            | positive number                                            | Flex grow ratio among siblings (CSS `flex-grow`). `grow="2"` and `grow="1"` produce a 2:1 split. `w="max"` / `h="max"` behave as `grow="1"`; when both are specified, `grow` takes precedence |
