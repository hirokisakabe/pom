...
export async function pptxToPng(
  ...
  try {
    ...
    execSync(
      `libreoffice --headless --convert-to png --outdir "${tempDir}" "${pptxPath}"`,
      { stdio: "inherit" },
    );
    ...
    console.log(`Generated: ${outputPngPath} (${pngFiles.length} slides)`);
  } finally {
    // 一時ディレクトリを削除
    if (fs.existsSync(tempDir)) {
      fs.rmSync(tempDir, { recursive: true });
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (!pptxPath || !outputPath) {
    console.error("Usage: npx tsx pptxToPng.ts <input.pptx> <output.png>");
    process.exit(1);
  }

  pptxToPng(pptxPath, outputPath);
}
