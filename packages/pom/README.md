...
- **Per-Side Borders** — `borderTop` / `borderRight` / `borderBottom` / `borderLeft` style each edge independently (e.g. `borderLeft.width="6"` for an accent bar, `borderBottom.width="3"` for an underlined heading), merging field-by-field with the uniform `border`. Combinable with `borderRadius`: `borderTop` / `borderBottom` paint their adjacent corner arcs (KPI-tile top-accent treatment), while `borderLeft` / `borderRight` keep their corners neutral so an accent bar reads clean. See [Nodes — Common Properties](./docs/nodes.md#common-properties).
